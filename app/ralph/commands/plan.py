...
from ..reconcile import reconcile_plan, dedup_tasks, extract_structured_output, ReconcileResult
...
def _commit_tix_plan(tix: Tix, spec_name: str, task_count: int, cwd: Path) -> bool:
    ...
    try:
        # ralph-state.json is ephemeral (lives in /tmp) — only commit plan
        subprocess.run(
...
def _finalize_plan(
    ...
    if config and retry > 0:
        def _plan_llm_fn(prompt: str) -> Optional[str]:
            out, _m, _s = _run_opencode(
                config, prompt, cwd, session_id=session_id
            )
            return out

        dedup_count = dedup_tasks(tix, _plan_llm_fn)
        if dedup_count > 0:
