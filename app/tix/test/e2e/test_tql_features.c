...
static void test_exec_is_null(TIX_TEST_ARGS()) {
  ...
  char err[256] = {0};
  tql_compiled_t c;
  /* model is a field that's actually NULL when not set (unlike spec which
     might be empty string). Use model= to test IS NULL. */
  ASSERT_OK(tql_prepare("tasks | model=", &c, err, sizeof(err)));
  ASSERT_STR_CONTAINS(c.sql, "IS NULL");

  /* Both tasks have no model, so both should match */
...
static void test_exec_is_not_null(TIX_TEST_ARGS()) {
  ...
  tix_ticket_t t1, t2;
  make_task(&t1, "T001", "Has model", TIX_STATUS_PENDING,
            TIX_PRIORITY_HIGH, "alice");
  snprintf(t1.model, TIX_MAX_NAME_LEN, "claude-sonnet-4-20250514");
  t1.cost = 1.5;

  make_task(&t2, "T002", "No model", TIX_STATUS_PENDING,
            TIX_PRIORITY_MEDIUM, "bob");
  /* t2.model left empty */
  ...
  ASSERT_OK(tql_prepare("tasks | cost!=", &c, err, sizeof(err)));
  ASSERT_STR_CONTAINS(c.sql, "IS NOT NULL");

  /* Only t1 has cost set */
