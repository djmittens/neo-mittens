...
Note: The textual library is imported lazily via create_textual_app() to avoid
startup cost and to allow the code to work when textual is not installed.
"""
...
__all__ = ["create_textual_app", "RalphDashboard"]
...
def create_textual_app():
    ...
    This function imports textual lazily to avoid startup cost and to allow
    the module to be imported even when textual is not installed.
            ...
            with Vertical(id="main-content"):
                with Vertical(id="info-panel"):
                    with Horizontal(id="ralph-section"):
                        yield RalphArtWidget(
                            RALPH_ART if RALPH_ART else [], id="ralph-art"
                        )
        ...
        def _update_status_panel(self):
            ...
            if state:
                ...
                status.progress_text = (
                    f"[green]Progress:[/] {done} done, {pending} pending"
                )
                ...
                issue_items = [("open", i.desc) for i in state.issues[:5]]
                issues_panel.update_issues(issue_items, len(state.issues), 0)
            else:
