...
LIBRARY_REMOTE="${LIBRARY_REMOTE:-/home/xyzyx/Calibre Library}"

# Bundle roots (where temporary export/import run directories live)
LOCAL_BUNDLES_ROOT="${LOCAL_BUNDLES_ROOT:-$HOME/CalibreSync}"
REMOTE_BUNDLES_ROOT="${REMOTE_BUNDLES_ROOT:-/home/xyzyx/CalibreSync}"
...
# Reads a stamp value or prints a default fallback (epoch date for initial full sync)
read_or_default_stamp() {
  local file="$1"; shift
  if [[ -f "$file" ]]; then
    cat "$file"
  else
    # No stamp = first sync, use epoch to capture all books
    echo "1970-01-01"
  fi
}
...
# Helper to run a bash script on remote with properly quoted env vars
remote_bash() {
  local script="$1"; shift
  # Build an env prefix that properly escapes values with spaces
  local env_prefix=""
  while (( $# >= 2 )); do
    local name="$1"
    local value="$2"
    shift 2
    # Single-quote the value to preserve spaces
    env_prefix+="${name}='${value}' "
  done
  ssh "$REMOTE_SSH" "${env_prefix}/usr/bin/env bash -c '${script}'"
}

echo "[calibre-ssh-sync] Starting two-way sync (laptop ↔ desktop)"

# Kill Calibre GUI if running (local)
if pgrep -x "calibre" >/dev/null 2>&1; then
  echo "[laptop] Stopping Calibre GUI..."
  pkill -x "calibre" || true
  sleep 2
fi

# Kill Calibre GUI if running (remote)
if ssh "$REMOTE_SSH" "pgrep -x calibre" >/dev/null 2>&1; then
  echo "[desktop] Stopping Calibre GUI..."
  ssh "$REMOTE_SSH" "pkill -x calibre" || true
  sleep 2
fi
...
if ! remote_bash '"$CALIBREDB" --version >/dev/null 2>&1' \
  CALIBREDB "$CALIBREDB_REMOTE_BIN"
then
  echo "Remote calibredb not found or not runnable: $CALIBREDB_REMOTE_BIN" >&2
  exit 1
fi
...
while IFS= read -r line; do
  [[ -z "$line" ]] && continue
  IDS_LOCAL+=("$line")
done < <("$CALIBREDB_LOCAL_BIN" --with-library "$LIBRARY_LOCAL" list \
...
remote_bash 'mkdir -p "$REMOTE_BUNDLES_ROOT/from-$PEER_LABEL_LOCAL" "$REMOTE_BUNDLES_ROOT/to-$PEER_LABEL_LOCAL"' \
  REMOTE_BUNDLES_ROOT "$REMOTE_BUNDLES_ROOT" \
  PEER_LABEL_LOCAL "$PEER_LABEL_LOCAL"
...
REMOTE_FROM_DATE=$(remote_bash \
  'if [[ -f "$STAMP_REMOTE_SEND" ]]; then cat "$STAMP_REMOTE_SEND"; else echo "1970-01-01"; fi' \
  STAMP_REMOTE_SEND "$STAMP_REMOTE_SEND")
REMOTE_SEND_MARK_NOW=$(ssh "$REMOTE_SSH" "date -u +%Y-%m-%dT%H:%M:%SZ")
...
while IFS= read -r line; do
  [[ -z "$line" ]] && continue
  IDS+=("$line")
done < <("$CALIBREDB" --with-library "$LIB" list --fields id --search "last_modified:>$FROM" | tail -n +2 | cut -d" " -f1 | grep -v "^$")
if (( ${#IDS[@]} )); then
  mkdir -p "$OUT"
...
' \
  ...
  PEER_LABEL_LOCAL "$PEER_LABEL_LOCAL" \
  STAMP_REMOTE_SEND "$STAMP_REMOTE_SEND" \
  REMOTE_SEND_MARK_NOW "$REMOTE_SEND_MARK_NOW"
...
for b in "$IN_ROOT"/run-*; do
  ...
  "$CALIBREDB" --with-library "$LIB" add --recurse "$b"
  rm -rf "$b"
done
' \
  CALIBREDB "$CALIBREDB_REMOTE_BIN" \
...
for bundle in "$LOCAL_IN_DIR"/run-*; do
  [[ -d "$bundle" ]] || continue
  [[ "$bundle" == *.imported.* ]] && continue
  echo "[laptop] Importing bundle: $bundle"
  "$CALIBREDB_LOCAL_BIN" --with-library "$LIBRARY_LOCAL" add --recurse "$bundle"
  rm -rf "$bundle"
done

# Clean up remote export bundles after successful import
remote_bash 'rm -rf "$REMOTE_BUNDLES_ROOT/to-$PEER_LABEL_LOCAL"/run-*' \
  REMOTE_BUNDLES_ROOT "$REMOTE_BUNDLES_ROOT" \
  PEER_LABEL_LOCAL "$PEER_LABEL_LOCAL"
